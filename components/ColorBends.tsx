...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (!isPausedRef.current) {
        ...
        const deg = (rotationRef.current % 360) + autoRotateRef.current * totalTime;
        ...
        const amt = Math.min(1, delta * pointerSmoothRef.current);
        cur.lerp(tgt, amt);
  ...
  return <div ref={containerRef} className={`fixed inset-0 z-0 overflow-hidden ${className}`} style={style} />;
}