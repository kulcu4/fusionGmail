...
const UserProfilePage: React.FC<UserProfilePageProps> = (props) => {
    ...
    const favoriteItems = useMemo((): MediaItem[] => favorites.map(src => {
    ...
    const ownedItems = useMemo((): MediaItem[] => {
        return collectionItems
            .filter(item => purchasedItems.includes(item.id))
            .map(item => ({ src: item.src, type: item.type })); // No styleId for owned items
            ...
            <main className="w-full flex flex-col items-center space-y-8 mt-6">
                ...
                {(activeTab === 'creations' || activeTab === 'favorites' || activeTab === 'owned') && (
                    <div className="w-full space-y-4 animate-fade-in">
                        ...
                        {activeTab === 'owned' && (ownedItems.length > 0 ? <MediaGrid items={ownedItems} onMediaClick={onMediaClick} selectionMode={selectionMode} selectedItems={selectedItems} onSelectItem={handleSelectItem} /> : <EmptyState message="Items you purchase from collections will appear here." />)}
                    </div>
                )}
            </main>
